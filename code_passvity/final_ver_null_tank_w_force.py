...
def main():
    ...
    # --------- find 3rd joint (for pulse torque) ----------
    joint3_name = "joint3"  # 如果XML里关节名不同，改这里
    ...
    dof3 = int(model.jnt_dofadr[jid3])  # joint3 对应的广义速度/力矩索引
    ...
    # main task
    t_hold = 0
    p0 = data.site_xpos[ee_site_id].copy()
    ...
    x2_hat = np.zeros(m2)
    x2_hat_inited = False
    E_tank, E_max, E_thresh = 15.0, 50.0, 35.0
    s_eps, alpha = 1e-8, 1
    s = np.sqrt(2 * E_tank)
    Kp_drift = np.array([10, 10, 10, 10, 10, 10, 10], dtype=float)

    # ============================================================
    # "Human" external torque pulse (apply at joint3 dof via qfrc_applied)
    # ============================================================
    tau_pulse_amp = 20  # [N·m] 脉冲幅值（ 5~20 ）
    pulse_T = 0.5  # [s] 每次脉冲持续时间
    pulse_gap = 5.0  # [s] 间隔
    n_pulses = 6
    t_first_pulse = 2.5
    ...
    with mujoco.viewer.launch_passive(model, data) as viewer:
        ...
        while viewer.is_running():
            ...
            # ============================================================
            # Apply "human" torque at joint3 via qfrc_applied
            # ============================================================
            data.qfrc_applied[:] = 0.0  # IMPORTANT: clear every step
            ...
            P_ext = float(np.dot(qd, data.qfrc_applied[:7]))  # W
            E_ext_int += P_ext * dt
    ...
    plt.title("Drift norm ||x2 - x2_hat||")
    plt.xlabel("time [s]")
