...
#
# In this depletion demo, alpha = 0: damping energy is dissipated and is not
# recovered into the tank. The tank is therefore operated without damping refill.
#
# Energy-tank passivation follows Dietrich, Ott, and Stramigioli, IEEE RA-L, 2016:
...
# ------------------------------------------------------------
# Energy tank -- depletion / intervention demo
#
# Goal:
#   Start slightly above the tapping threshold so the classical-like
#   secondary behavior is initially available.  Set ALPHA = 0 so
#   controller damping is NOT refilled into the tank.  Whenever
#   u^T F2 < 0 the tank is consumed.  Once Etank reaches E_THRESH,
#   allow_tap becomes False, u -> 0, and the secondary-task
#   performance is intentionally degraded to preserve the energy budget.
#
# This corresponds to the "tank without refilling" type of experiment.
# ------------------------------------------------------------
E_TANK_INIT = 5.010
E_MAX = 50.0
E_THRESH = 4.9800
...
def main():
    ...
    # First detected transition from tank-available to tank-blocked.
    tap_was_available = None
    tap_off_time = None
    ...
    with mujoco.viewer.launch_passive(model, data) as viewer:
        ...
        while viewer.is_running():
            ...
            # Detect the intervention instant Etank -> threshold.
            if tap_was_available is None:
                tap_was_available = bool(allow_tap)
            elif tap_was_available and (not allow_tap) and tap_off_time is None:
                tap_off_time = t
                print(
                    f"[TANK INTERVENTION] t={t:.4f} s, "
                    f"Etank={E_tank:.9f} J <= threshold={E_THRESH:.9f} J; "
                    "u is now disabled."
                )
            tap_was_available = bool(allow_tap)
            ...
            if total_steps % LOG_EVERY == 0:
                ...
                allow_tap_log.append(1.0 if allow_tap else 0.0)
    ...
    allow_tap_log = np.asarray(allow_tap_log)
    ...
    # Mark the exact tank intervention time on task plots.
    if tap_off_time is not None:
        for ax in [ax2, ax3, ax4, ax5, ax6]:
            ax.axvline(tap_off_time, linestyle=":", linewidth=1.5, label="tank intervention")
        # Keep legends only where useful.
        ax3.legend()
        ax4.legend()
        ax6.legend()
    ...
    if tap_off_time is not None:
        for ax in axes:
            ax.axvline(tap_off_time, linestyle=":", linewidth=1.5)
    ...
    # ========================================================
    # Figure 4: depletion / intervention story
    # ========================================================
    fig4, axes4 = plt.subplots(4, 1, figsize=(14, 13))

    axes4[0].plot(T_log, Etank_log, label="Etank")
    axes4[0].axhline(E_THRESH, linestyle="--", label="threshold")
    axes4[0].set_ylabel("energy [J]")
    axes4[0].set_title("Tank depletion and intervention")
    axes4[0].grid(True)
    axes4[0].legend()

    axes4[1].step(T_log, allow_tap_log, where="post", label="allow_tap")
    axes4[1].set_ylim(-0.1, 1.1)
    axes4[1].set_yticks([0, 1])
    axes4[1].set_yticklabels(["blocked", "available"])
    axes4[1].set_ylabel("tank state")
    axes4[1].grid(True)
    axes4[1].legend()

    axes4[2].plot(T_log, drift_log * 1000.0, label="||x2-x2_hat||")
    axes4[2].set_ylabel("drift [mm]")
    axes4[2].set_title("Virtual-coordinate drift")
    axes4[2].grid(True)
    axes4[2].legend()

    axes4[3].plot(T_log, tip_err_actual_log * 1000.0, label="actual tip tracking error")
    axes4[3].set_xlabel("time [s]")
    axes4[3].set_ylabel("error [mm]")
    axes4[3].set_title("Secondary-task performance")
    axes4[3].grid(True)
    axes4[3].legend()

    if tap_off_time is not None:
        for ax in axes4:
            ax.axvline(tap_off_time, linestyle=":", linewidth=1.8, label="tank intervention")
    ...
    if tap_off_time is None:
        print("tank intervention time              = NOT REACHED")
    else:
        print(f"tank intervention time              = " f"{tap_off_time: .6f} s")

        pre_mask = T_log < tap_off_time
        post_mask = T_log >= tap_off_time

        if np.any(pre_mask):
            pre_tip_rms = np.sqrt(np.mean(tip_err_actual_log[pre_mask] ** 2))
            pre_drift_rms = np.sqrt(np.mean(drift_log[pre_mask] ** 2))
            print(f"pre-intervention tip RMS            = " f"{pre_tip_rms*1000.0: .6f} mm")
            print(f"pre-intervention drift RMS          = " f"{pre_drift_rms*1000.0: .6f} mm")

        if np.any(post_mask):
            post_tip_rms = np.sqrt(np.mean(tip_err_actual_log[post_mask] ** 2))
            post_drift_rms = np.sqrt(np.mean(drift_log[post_mask] ** 2))
            print(f"post-intervention tip RMS           = " f"{post_tip_rms*1000.0: .6f} mm")
            print(f"post-intervention drift RMS         = " f"{post_drift_rms*1000.0: .6f} mm")
    ...
    print(
        "3) Depletion demo: before Etank reaches E_THRESH, u ~= w1_ref and "
        "the controller behaves close to the classical null-space solution."
    )

    print(
        "4) At the intervention instant Etank <= E_THRESH, allow_tap switches "
        "to False, u becomes zero, Etank should stop being consumed (ALPHA=0), "
        "and secondary-task drift/error should increase while the RCM task "
        "remains prioritized."
    )

    print(
        "5) The corrected energy residual should remain close to zero on both "
        "sides of the intervention."
    )
